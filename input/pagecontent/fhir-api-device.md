...
| **Search Parameters** | The resource server MUST be able to discover the patient identifier from the Access Token. This identifier MUST implicitly be considered as the `subject` argument with every _search_ request. If a DiGA explicitly provides a `subject` argument with a query, the resource server MUST ignore this argument and SHOULD respond with an `400 Bad Request` error.<br>The resource server MAY support additional standard FHIR Device search parameters. |
