...
| **Authorization** | OAuth2 Bearer token required. The resource server MUST only return a Device resource that is linked to the patient in the token and that is accessible by the requestor according to its scopes (see [Smart Scopes](smart-scopes.html) for the compartment semantics that MUST be used for validating the authorization of the requestor).  |
