...
| `404` | - FHIR-Read on a resource that does not exist on the resource server<br>- FHIR-Read on a resource that the DiGA is not allowed to access due to OAuth scope restrictions | [OperationOutcome](https://hl7.org/fhir/R4/operationoutcome.html) where:<br>- `issue.severity` set to `error`<br>- `issue.code` set to `processing`<br>- `issue.diagnostics` set to `Resource %s is not known`. |
