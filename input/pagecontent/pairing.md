...
|----------------------------|------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **client_id**              | Unique identifier of the DiGA on application level used in OAuth 2.0 flows.  | MUST correspond to the value stored in the [_DiGA-VZ_](registries-and-zts.html#diga-verzeichnis) and follow the structure `urn:diga:bfarm:{DiGA-ID}` where `{DiGA-ID}` is the unique five-digit identifier of the DiGA as published in the [_DiGA-VZ_](registries-and-zts.html#diga-verzeichnis).                                                                                                                            |
...
| **redirect_uri**           | Redirect target for the OAuth 2.0 Authorization Code Flow.                   | Serves as a security anchor ensuring that authorization codes are returned only to registered and verified DiGA endpoints. MUST exactly match the URI registered in the [_DiGA-VZ_](registries-and-zts.html#diga-verzeichnis) (strict comparison MUST be performed).                                                                                                             |
| **tls_client_certificate** | TLS client certificate identifying the DiGA backend for mTLS authentication. | MUST uniquely identify the DiGA backend. The certificate typically renews annually or earlier if revoked. The OAuth2 Authorization Server MUST validate that the presented certificate matches the one registered in the [_DiGA-VZ_](registries-and-zts.html#diga-verzeichnis) at connection time. A certificate MAY be cached (see [Caching of Trust-Related Information](security-and-privacy.html#caching-of-trust-related-information)).                                             |

The provider of the Device Data Recorder MUST ensure that all client registrations remain synchronized with the [_DiGA-VZ_](registries-and-zts.html#diga-verzeichnis). If a DiGA is
...
The Authorization Server MUST validate requested scopes against the [_DiGA-VZ_](registries-and-zts.html#diga-verzeichnis) entry for the client and strictly verify
...
| **Unique**              | The Pairing ID MUST be unique for each patient at least within each Device Data Recorder. <br>If the same patient pairs with multiple DiGAs, each pairing MUST result in a distinct Pairing ID.                                                                                                                                                                                                                                                         |
| **Unpredictable**       | The Pairing ID MUST NOT be guessable; it must be sufficiently long and unpredictable.                                                                                                                                                                                                                                                                                                                                                                   |
| **Anonymous**           | The Pairing ID MUST NOT allow any conclusions about the real-world identity of the patient.                                                                                                                                                                                                                                                                                                                                                             |
| **Stable and Retained** | The Pairing ID MUST be created once during the pairing sequence after the patient has authenticated and given consent. <br>It MUST remain stable for the entire lifetime of the pairing and MUST be retained by the Device Data Recorder. If a DiGA processes multiple MIVs and obtains consent for additional MIVs at different points in time, the same Pairing ID MUST continue to be used and MUST NOT be re-created or changed on a per-MIV basis. |
...
status. The following scenarios are described in detail below.
