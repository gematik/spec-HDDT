...
Device Data Recorders MAY limit access to historical data to a MIV-spevific `Historic Data Period`, e.g. 30 days (see [Properties of Device Data Recorders](information-model.html#miv-specific-properties-of-device-data-recorders) for details). In case a DiGA requests data that is older than the `Historic Data Period`, the Device Data Recorder MUST respond with a _404 Not Found_ error (see [https://hl7.org/fhir/R4/http.html#read](https://hl7.org/fhir/R4/http.html#read) for details on handling deleted resources).
