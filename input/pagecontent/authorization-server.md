...
The `access_token` issued by the Authorization Server is used as a **Bearer token** in the HTTP `Authorization` header for all requests from the DiGA to the [FHIR Resource Server](ddr-diga-api.html). Resource Servers validate this token and enforce the granted [SMART scopes](smart-scopes.html) before returning any data.
