...
|-------------------|-----------------|---------|
| blood glucose measurement ([Observation](https://hl7.org/fhir/R4/observation.html)) | [UK Core Observation Blood Glucose](https://simplifier.net/hl7fhirukcorer4/ukcore-observation-bloodglucose) | UK Core allows only SNOMED CT for the `code` element while HDDT only allows LOINC. <br>`device` is optional for UK Core. |
...
| | [International Patient Summary: Observation Results](https://hl7.org/fhir/uv/ips/STU1.1/StructureDefinition-Observation-results-uv-ips.html) | `device` is optional for IPS.<p>Other derivations are minor issues that do not affect the compatibility with HDDT blood sugar measurements: IPS only allows for _final_ as an observation's `status`. IPS allows for explicitly missing effective[x] elements using an extension. |
...
| | [KBV Basisprofile](https://simplifier.net/base1x0/kbv_pr_base_observation_glucose_concentration) (Germany) | `device` is mandatory with HDDT. HDDT only allows for a single `code` while KBV requests two (LOINC and SCT). |
| | [ISiKLebensZustand](https://simplifier.net/guide/isik-basis-stufe-5/Einfuehrung/Artefakte/Datenobjekte_Lebenszustand?version=5.0.0) (Germany) | `device` is mandatory with HDDT. HDDT only allows for a single LOINC `code` while ISiK allows for an additional SCT code. ISiK sets `value[x]`to [1..1], while HDDT allows for values to be absent. |
| | [Finnish PHR Blood Glucose Profile](https://simplifier.net/digious-fi/fiphrsdbloodglucose) | Finnish PHR allows for LOINC Codes which are not part  of the HDDT MIV. Finnish PHR disallows `device` which is mandatory for HDDT.  |
