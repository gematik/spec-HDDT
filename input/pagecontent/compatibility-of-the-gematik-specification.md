...
| | [International Patient Summary: Observation Results](https://hl7.org/fhir/uv/ips/STU1.1/StructureDefinition-Observation-results-uv-ips.html) | `Observation.device` is optional for IPS.<p>Other derivations are minor issues that do not affect the compatibility with HDDT blood sugar measurements: IPS only allows for _final_ as an observation's `status`. IPS allows for explicitly missing effective[x] elements using an extension. |
