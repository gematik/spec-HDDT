...
| **Search Parameters** | A full list of the **search parameters** that this endpoint MUST support can be found on page [FHIR Resource Server - Search Parameters](ddr-diga-api.html#search-parameters).<br><br>• The resource server MAY support any additional standard FHIR DeviceMetric search parameters.<br>• A DiGA MAY, for example, use the `source` parameter to discover all DeviceMetric resources that belong to a given Personal Health Device instance. |
